...
已正式上線：**《岳陽樓記》**（范仲淹）、**《師說》**（韓愈）、**《論仁、論孝、論君子》**（《論語》）、**《魚我所欲也》**（孟子）、**《逍遙遊》**（節錄）（莊子）。其餘篇章於首頁顯示「準備中」。
...
│               ├── content.json        （17題：內容理解）
│               ├── structure-skill.json（4題：結構／手法／修辭）
│               ├── theme.json          （1題：主旨與思考，開放題）
│               └── cross-text.json     （4題：跨篇比較，對象包括《念奴嬌》《勸學》《月下獨酌》《魚我所欲也》）
└── assets/
    └── audio/                      ← 誦讀音檔（見下方對照表）
...
這是你提供的「DSE十二篇字詞表」整份解析結果，按篇存放，不會被網站程式讀取（純粹是暫存參考資料，不影響網站運作）。已建立的五篇（《岳陽樓記》《師說》《論仁、論孝、論君子》《魚我所欲也》《逍遙遊》）中，缺漏的字詞已直接補進對應的 `text.json`；其餘 11 篇（六國論、青玉案‧元夕、山居秋暝、念奴嬌‧赤壁懷古、月下獨酌、登樓、出師表、廉頗藺相如列傳、勸學、始得西山宴遊記、聲聲慢）尚未建立完整內容，日後製作這些篇章時，可直接從這個檔案取用字詞表資料，不需要重新上傳或重新解析。
