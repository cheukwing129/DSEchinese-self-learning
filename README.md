...
已正式上線：**《岳陽樓記》**（范仲淹）、**《師說》**（韓愈）、**《論仁、論孝、論君子》**（《論語》）、**《魚我所欲也》**（孟子）、**《逍遙遊》**（節錄）（莊子）、**《勸學》**（節錄）（荀子）、**《廉頗藺相如列傳》**（節錄）（司馬遷）、**《六國論》**（蘇洵）、**《出師表》**（諸葛亮）。其餘篇章於首頁顯示「準備中」。
...
│       ├── shishuo/              （結構同下，共 60 題）
│       ├── lunyu-renxiaojunzi/   （結構同下，共 61 題）
│       ├── yu-wo-suoyu/          （結構同下，共 55 題）
│       ├── xiaoyaoyou/           （結構同下，共 53 題）
│       ├── quanxue/              （結構同下，共 64 題）
│       ├── lianpo-linxiangru/    （結構同下，共 60 題）
│       ├── liuguolun/            （結構同下，共 42 題）
│       └── chushibiao/
│           ├── unit.json           ← 篇章 meta（模組清單、跨篇對象、題庫檔案清單、音檔路徑）
│           ├── text.json           ← 原文 7 段 + 47 條教育局注釋
...
│           └── question-banks/
│               ├── words.json          （33題：字詞／虛詞，含跨篇虛詞辨析）
│               ├── content.json        （15題：內容理解）
│               ├── structure-skill.json（5題：結構／手法）
│               ├── theme.json          （2題：主旨與思考，開放題）
│               └── cross-text.json     （4題：跨篇比較，對象包括《廉頗藺相如列傳》《論仁論孝論君子》《六國論》）
└── assets/
    └── audio/                      ← 誦讀音檔（見下方對照表）

data/_staging/dse-12-vocab-table.json  ← 十二篇字詞表原始資料（尚未拆入各篇 text.json 的部分，見下方說明）
```

### `data/_staging/dse-12-vocab-table.json` 說明

這是你提供的「DSE十二篇字詞表」整份解析結果，按篇存放，不會被網站程式讀取（純粹是暫存參考資料，不影響網站運作）。已建立的九篇（《岳陽樓記》《師說》《論仁、論孝、論君子》《魚我所欲也》《逍遙遊》《勸學》《廉頗藺相如列傳》《六國論》《出師表》）中，缺漏的字詞已直接補進對應的 `text.json`；其餘 7 篇（青玉案‧元夕、山居秋暝、念奴嬌‧赤壁懷古、月下獨酌、登樓、始得西山宴遊記、聲聲慢）尚未建立完整內容，日後製作這些篇章時，可直接從這個檔案取用字詞表資料，不需要重新上傳或重新解析。
...
### `text.json` 資料模型備註（日後新增篇章適用）

- **段落分組（`section` 欄位）**：若某段落物件加上 `"section": "論仁"` 這類欄位，「原文與誦讀」頁面會自動改為按 `section` 分組導覽（如《論仁、論孝、論君子》分成「論仁／論孝／論君子」三個分頁），而非逐段（第1段/第2段…）導覽。適合語錄體、輯錄體等非單一敘事結構的篇章；一般記敍文（如《岳陽樓記》《師說》）不需要此欄位，會維持逐段導覽。
- **重複詞語注釋（`occurrence` 欄位）**：若同一段內有兩個完全相同的詞語，但意思不同（例如《論仁》中兩次出現的「不以其道得之」，分別指「富貴」和「貧困」），可為注釋加上 `"occurrence": 1` 或 `"occurrence": 2`（預設為 1），分別建立兩條注釋，指定各自對應第幾次出現的位置，避免兩處被誤標成同一個解釋。
